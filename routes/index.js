...
router.post("/edit-user/:id", ensureAuthenticated, async (req,res) => {
    try{
        const {id} = req.params;
        const {balance, total_deposit, active_deposit, total_withdraw, debt, account_plan} = req.body;
        const customer = await User.findOne({_id:id}).limit(10)
        if(!balance || !total_deposit || !active_deposit || !total_withdraw || !debt || !account_plan){
